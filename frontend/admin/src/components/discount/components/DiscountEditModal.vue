...
const discountType = ref("percentage"); // Default to percentage
...
// Initialize form with discount data and determine the discount type
watch(
	() => props.discount,
	(newDiscount) => {
		if (newDiscount) {
			...
			discountForm.discountAmount = newDiscount.discountAmount || 0;
			discountForm.startDate = formatDateForInput(
				new Date(newDiscount.startDate)
			);
			discountForm.endDate = formatDateForInput(
				new Date(newDiscount.endDate)
			);

			// Determine the discount type based on the values
			if (newDiscount.discountPercentage > 0) {
				discountType.value = "percentage";
			} else if (newDiscount.discountAmount > 0) {
...
async function handleSubmit() {
	...
	try {
		...
		emit("update", props.discount.id, formData);
	} catch (error) {
